document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('form-reformas').addEventListener('submit', async function(e){
        ...
        try {
            const response = await fetch('https://script.google.com/macros/s/AKfycbzrGV_j-adxfotub4bwJNFveFSp01BypuDbLaZLC8-jWCpGxZ52YVrcCSSKCeq0PUU5/exec', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });

            // 🔴 CLAVE: ver qué devuelve Google
            const text = await response.text();
            console.log("RESPUESTA RAW:", text);

            // 🔴 CLAVE: parsear manualmente
